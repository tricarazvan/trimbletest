...
public class IssuesPage {
	...
	/* Open Bttn */
	// private By OpenBttn = By.xpath("//a[contains(@class, 'btn-link') and
	// contains(., 'Open')]");

	// private By OpenBttn = By.cssSelector("a.btn-link.selected:contains('open')");
	// private By OpenBttn = By.xpath("//div[contains(@class,
	// 'table-list-header-toggle') and contains(@class, 'states') and
	// contains(@class, 'flex-auto')]");
	...
	// private By OpenBttn = By.xpath("//div[contains(@class, 'd-block d-lg-none
	// no-wrap')]//svg[contains(@class, 'octicon octicon-issue-opened')]");

	/* issue Title */
	// private By TitleIssue = By.xpath("//div[contains(@class, 'flex-auto
	// min-width-0 p-2 pr-3 pr-md-2')][1]/a]");
	private By TitleIssue = By
			.xpath("(//a[@class='Link--primary v-align-middle no-underline h4 js-navigation-open markdown-title'])[1]");

	/* Issue Lable */
	// private By Labels =

	/* Issue Comments */
	private By Comments = By.xpath(
			"//a[contains(@class, 'Link--muted') and contains(@aria-label, 'comments') and @data-turbo-frame='repo-content-turbo-frame']");
...
//		driver.findElement(labelFilterInput).sendKeys(Keys.RETURN);
	...
	public void clickOnOpenBttn() {
		...
		try {
			...
			// If StaleElementReferenceException is caught, re-locate the element and retry
			// the action
